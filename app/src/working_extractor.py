...
class WorkingPDFOutlineExtractor:
    """Working PDF outline extractor with ML capabilities"""
    ...
    def __init__(self, model_path: str = "models/"):
        ...
        try:
            global RandomForestClassifier, TfidfVectorizer, StandardScaler, LabelEncoder
            global train_test_split, classification_report, accuracy_score, joblib
    ...
    def extract_numerical_features(self, features: List[TextFeatures]) -> np.ndarray:
        ...
        for feature in features:
            row = [
                ...
                abs(hash(feature.font_name)) % 1000
            ]
    ...
    def train_models(self, training_data_dir: str):
        ...
        if not self.ml_available:
            raise ValueError("ML libraries not available. Cannot train models.")
    ...
    def predict_headings_ml(self, features: List[TextFeatures]) -> List[str]:
        """Predict heading levels using ML models"""
        if not self.ml_available or not self.heading_classifier or not self.tfidf_vectorizer:
            raise ValueError("ML models not available or not trained.")
    ...
    def predict_headings_rules(self, features: List[TextFeatures]) -> List[str]:
        """Predict headings using rule-based approach"""
        predictions = []
        ...
        for feature in features:
            ...
            # Font size factor
            if feature.font_size > avg_font_size * 1.5:
                score += 30
            elif feature.font_size > avg_font_size * 1.2:
                score += 20
            elif feature.font_size > avg_font_size * 1.1:
                score += 10
            
            # Formatting factors
            if feature.is_bold:
                score += 25
            if feature.is_underline:
                score += 15
            if feature.is_italic:
                score += 5
            
            # Length factors
            if feature.word_count <= 8:
                score += 20
            elif feature.word_count <= 15:
                score += 10
            
            # Position factors
            if feature.position_x < 0.2:  # Left-aligned
                score += 10
            
            # Content factors
            if feature.is_uppercase:
                score += 15
            elif feature.is_titlecase:
                score += 10
            
            # Numbered patterns
            if re.match(r'^\d+\.', feature.text):
                score += 20
            elif re.match(r'^\d+\.\d+', feature.text):
                score += 15
            
            # Classify based on score
            if score >= 60:
                predictions.append('H1')
            elif score >= 40:
                predictions.append('H2')
            elif score >= 25:
                predictions.append('H3')
            elif score >= 15:
                predictions.append('H4')
            else:
                predictions.append('BODY')
        
        return predictions
    
    def extract_title(self, features: List[TextFeatures]) -> str:
        """Extract title from features"""
        title_candidates = []
        
        for feature in features:
            if feature.page_number == 1:
                score = 0
                score += feature.font_size * 2
                if feature.is_bold:
                    score += 30
                if feature.is_underline:
                    score += 20
                if 2 <= feature.word_count <= 15:
                    score += 20
                if feature.position_y < 0.3:  # Top of page
                    score += 15
                
                title_candidates.append((score, feature.text))
    ...
    def process_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.ml_available and self.heading_classifier:
                predictions = self.predict_headings_ml(features)
                logger.info("Using ML-based predictions")
            else:
                predictions = self.predict_headings_rules(features)
                logger.info("Using rule-based predictions")
            
            # Extract title
            title = self.extract_title(features)
...
def main():
    parser = argparse.ArgumentParser(description='Working PDF outline extraction')
    ...
    try:
        extractor = WorkingPDFOutlineExtractor(model_path=args.model_path)
